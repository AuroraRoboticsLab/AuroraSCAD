...
//finesse options
  ...
  convexity=6 // reduce render errors
) {
  z = is_undef(w) ? z : w;
  ...
  if (chamfer == 0) polyhedron(points=pts, faces=concat(side, caps), convexity=convexity);
  else render(10) I() {
    polyhedron(points=pts, faces=concat(side, caps), convexity=convexity);
